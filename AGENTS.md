...
- 测试应尽量贴近真实业务边界：优先验证项目如何使用真实依赖 / SDK，而不是为测试重新定义一套简化模型。
- 测试辅助函数只在能表达稳定领域概念或消除重复时引入；不要为了断言方便创建只服务单个测试的中间抽象。
- 测试拆分服务于行为理解，而不是形式化颗粒度。强相关的输入对照可以放在同一个测试里，让因果关系更直接。
- 当错误会丢失重要上下文时，先记录结构化诊断信息再抛出，让调用方和测试报告都能定位问题。

### 从用户修改中学习偏好

- 从用户修改中学习偏好时，要区分“用户实际改变了什么”和“当前工作区最终长什么样”。对照上一版实现、编辑记录和对话上下文，避免把 agent 自己的改动误归因给用户。
- 总结偏好时上提一层抽象：提炼稳定的工程取向和可复用原则，而不是记录某次改动的表面形式。
